...
<?php if ( ! WC()->cart->is_empty() ) : ?>
    ...
    <div class="woocommerce-mini-cart cart_list product_list_widget <?php echo esc_attr( $args['list_class'] ); ?>">
        ...
        foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
            ...
            if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
                ...
                $thumbnail         = apply_filters( 'woocommerce_cart_item_thumbnail', $_product->get_image('woocommerce_thumbnail', $default_attr), $cart_item, $cart_item_key );
                        ...
                        <div class="item-image col-3">
                            <div class="mt-1">
                                <?php if ( empty( $product_permalink ) ) : ?>
                                    <?php echo $thumbnail2; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                                <?php else : ?>
    ...
    <div class="cart-footer bg-light text-center position-absolute bottom-0 p-3 w-100">
        ...
        <p class="text-muted small shipping-text"><?php esc_html_e('Gå videre for fraktkostnader.' , 'brimo'); ?></p>
